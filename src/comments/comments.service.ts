...
export class CommentsService {
  ...
  async createCommentaty(
    user_id: number,
    card_id: number,
    dto: CreateCommentaryDTO,
  ): Promise<Commentary> {
    ...
    return this.commentsRepository.save(commentary);
  }
  ...
  async updateCommentary(
    ...
    card_id: number,
    id: number,
    dto: UpdateCommentaryDTO,
  ): Promise<Commentary> {
    const commentary = await this.getOneCommentaty(
      user_id,
      column_id,
      card_id,
      id,
